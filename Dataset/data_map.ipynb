...
df = df[(df['longitude'] >= 116) & (df['longitude'] <= 116.8) &
        (df['latitude'] >= 39.3) & (df['latitude'] <= 40.50)]

# Load edge server locations
edge_servers = pd.read_csv('edge_server_locations_.csv')
...
plt.tight_layout()
plt.savefig('taxi_points_with_edge_servers_.png', dpi=300, bbox_inches='tight')
plt.show()