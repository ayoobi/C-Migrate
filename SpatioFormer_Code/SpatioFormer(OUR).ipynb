# Transformer-based trajectory prediction model (SpatioFormer)
import numpy as np
import pandas as pd
import torch
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None):
        # Self attention
        tgt2, self_attn = self.self_attn(tgt, tgt, tgt, tgt_mask)
...
# Simplified Transformer-based Trajectory Prediction Model
class TrajectoryTransformer(nn.Module):
    def __init__(self, input_dim=2, d_model=128, n_heads=8, 
        ...
        # Transformer encoder layers only (simpler architecture)
    ...
    def forward(self, src):
        ...
        for layer in self.encoder_layers:
            memory, attn_weights = layer(memory)
...
def train_transformer_model(model, train_loader, val_loader, criterion, optimizer, 
...
def process_single_taxi_file_transformer(file_path):
    """Process a single taxi file using the Transformer approach"""
    ...
    print(f"\nTransformer Results for {os.path.basename(file_path)}:")
    print("=" * 50)
...
def main():
    ...
    for file_name in file_list[:10]:  # Process first 3 files for demonstration
        ...
        metrics = process_single_taxi_file_transformer(file_path)
    ...
    if all_metrics:
        print("\n" + "="*60)
        print("TRANSFORMER AGGREGATE RESULTS ACROSS ALL TAXIS")
        print("="*60)
        ...
        print(f"\nAverage Transformer Performance Metrics:")
        print(f"Regression Metrics:")
