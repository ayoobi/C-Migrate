#Final CNN based Trajectory Prediction 
import numpy as np
import pandas as pd
import torch
...
def analyze_convergence(convergence_data):
    ...
    convergence_metrics = {
        ...
        'best_val_loss': best_val_loss,
        'convergence_epoch': best_val_epoch,  # When best validation was achieved
        'overfitting_gap': final_train_loss - final_val_loss,  # Negative means overfitting
    }
...
def process_single_taxi_file(file_path):
    """Process a single taxi file using the CNN approach"""
...
def main():
    ...
    for file_name in file_list[:3]:  # Process first 3 files for demonstration
    ...
    if all_metrics and all_convergence:
        print("\n" + "="*80)
        print("AGGREGATE RESULTS ACROSS ALL TAXIS")
        print("="*80)
